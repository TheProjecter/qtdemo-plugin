...
//    mbox.exec();
    ...
    case QLibraryInfo::DocumentationPath: { path += QString(QLatin1String("/doc")); break; }
    case QLibraryInfo::HeadersPath: { path += QString(QLatin1String("/include")); break; }
    case QLibraryInfo::LibrariesPath: { path += QString(QLatin1String("/lib")); break; }
    case QLibraryInfo::BinariesPath: { path += QString(QLatin1String("/bin")); break; }
    case QLibraryInfo::PluginsPath: { path += QString(QLatin1String("/plugins")); break; }
    case QLibraryInfo::DataPath: { path += QString(QLatin1String("/data")); break; }
    case QLibraryInfo::TranslationsPath: { path += QString(QLatin1String("/translations")); break; }
    case QLibraryInfo::SettingsPath: { path += QString(QLatin1String("/settings")); break; }
...
void MenuManager::initHelpEngine()
{
    ...
    QString qtDocRoot = QtLibraryLocation(QLibraryInfo::DocumentationPath) + QLatin1String("/qch");
...
void MenuManager::itemSelected(int userCode, const QString &menuName)
{
    switch (userCode){
    case PROJECT:
        this->openProject(this->currentInfo);
        break;
    case LAUNCH:
    ...
    case DOCUMENTATION:
        ...
        break;
    case QUIT:
        if (QMessageBox::Ok == QMessageBox::question(window,
                                                     tr("QtCreater"),
                                                     tr("Are you sure to quit QtCreator?"),
...
void MenuManager::showDocInAssistant(const QString &name)
{
    ...
    if (this->assistantProcess.state() != QProcess::Running){
        QString app = QtLibraryLocation(QLibraryInfo::BinariesPath) + QDir::separator();
#if !defined(Q_OS_MAC)
...
void MenuManager::launchExample(const QString &name)
{
    ...
    env.insert(QLatin1String("PATH"), QtLibraryLocation(QLibraryInfo::BinariesPath)
...
QString MenuManager::resolveDataDir(const QString &name)
{
    ...
    QDir dir;
    if (category == "demos")
        dir = QDir(QtLibraryLocation(QLibraryInfo::DemosPath));
    else
        dir = QDir(QtLibraryLocation(QLibraryInfo::ExamplesPath));
...
QString MenuManager::resolveProFile(const QString &name)
{
    QString dirName = this->info[name]["dirname"];
    QString category = this->info[name]["category"];
    QString fileName = this->info[name]["filename"];
    ...
    QFile proFile(dir.path() + "/" + fileName + ".pro");
    if (proFile.exists()) return proFile.fileName();

    return "__project file not found__";
}
...
void MenuManager::createLeafMenu(const QDomElement &el)
{
    ...
    if (el.attribute("executable") != "false")
    {
        ...
        createLowRightLeafButton2("Open project", 600, PROJECT, infoButtonsIn, infoButtonsOut, 0);
    }
}
...
void MenuManager::createLowRightLeafButton2(const QString &label, int xOffset, BUTTON_TYPE type, Movie *movieIn, Movie *movieOut, Movie * /*movieShake*/)
{
    TextButton *item = new TextButton(label, TextButton::RIGHT, type, this->window->scene, 0, TextButton::PANEL);
    item->setRecursiveVisible(false);
    item->setZValue(10);
    ...
    // create in-animation:
    DemoItemAnimation *anim = new DemoItemAnimation(item, DemoItemAnimation::ANIM_IN);
    anim->setDuration(2050 * Colors::animSpeedButtons);
    anim->setStartPos(QPointF(sw, Colors::contentStartY + Colors::contentHeight - (targetPos + 9)));
    anim->setPosAt(0.10, QPointF(sw, Colors::contentStartY + Colors::contentHeight - (targetPos + 9)));
    anim->setPosAt(0.30, QPointF(xOffset, Colors::contentStartY + Colors::contentHeight - (targetPos + 9)));
    anim->setPosAt(0.25, QPointF(xOffset + 30, Colors::contentStartY + Colors::contentHeight - (targetPos + 9)));
    anim->setPosAt(0.40, QPointF(xOffset, Colors::contentStartY + Colors::contentHeight - (targetPos + 9)));
    anim->setPosAt(0.45, QPointF(xOffset + 5, Colors::contentStartY + Colors::contentHeight - (targetPos + 9)));
    anim->setPosAt(0.50, QPointF(xOffset, Colors::contentStartY + Colors::contentHeight - (targetPos + 9)));
    anim->setPosAt(1.00, QPointF(xOffset, Colors::contentStartY + Colors::contentHeight - targetPos));
    movieIn->append(anim);

    // create out-animation:
    anim = new DemoItemAnimation(item, DemoItemAnimation::ANIM_OUT);
    anim->hideOnFinished = true;
    anim->setDuration(200 * Colors::animSpeedButtons);
    anim->setStartPos(QPointF(xOffset, Colors::contentStartY + Colors::contentHeight - targetPos));
    anim->setPosAt(1.0, QPointF(xOffset, sh));
    movieOut->append(anim);
}
